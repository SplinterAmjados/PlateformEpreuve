...
class ChallengeController extends Controller {
	...
	public function validateAction($id) {
		...
		$manager = $this->getDoctrine()->getManager();
		$id_user = $this->container->get("security.context")->getToken()
				->getUser()->getId();
		try {
		...
		if (!$request->isXmlHttpRequest()) // requet qui vienne d'une autre place
 {
	...
	public function challengerInfoAction(){
		$rank = 0; 
		if ($this->get('security.context')->isGranted('ROLE_CHALLENGER')) {
	 ...
	 */
	public function getFlagFromAction()
	{
	 ...
	 */
	public function validateFlagAction()
	{
		$request = $this->getRequest();
		...
		$user = $manager->getRepository("SecurinetsUsersBundle:User")->find($this->get('security.context')->getToken()->getUser()->getId());
